"use client";

import { Button } from "@relume_io/relume-ui";
import React from "react";
import { RxChevronRight } from "react-icons/rx";

export function Layout149() {
    return (
        <section id="aviana" className="px-[5%] py-8 md:py-12 lg:py-14">
            <div className="container">
                <div className="flex flex-col items-center">
                    <div className="mb-12 md:mb-18 lg:mb-20">
                        <div className="mx-auto flex max-w-lg flex-col items-center text-center">
                            <h2 className="rb-5 mb-5 text-5xl font-bold md:mb-6 md:text-7xl lg:text-8xl">
                                Team Building That Creates Real Cohesion
                            </h2>
                            <p className="mb-5 md:mb-6 md:text-md">
                                Strong teams are built through trust, clarity, and shared
                                ownership not activities alone.
                            </p>
                            <p className="mb-5 md:mb-6 md:text-md">
                                Our team-building workshops are experiential, reflective, and
                                outcome-driven, designed to improve collaboration and working
                                relationships.
                            </p>
                            <div className="py-2 text-left">
                                <h3 className="mb-3 font-bold text-md text-center">
                                    Focus Areas
                                </h3>
                                <ul className="list-disc pl-5 space-y-2 inline-block">
                                    <li>Team Communication & Collaboration</li>
                                    <li>Interpersonal Skills (FIRO-B Based)</li>
                                    <li>Trust Building & Role Clarity</li>
                                    <li>Cross-Functional Effectiveness</li>
                                </ul>
                            </div>

                        </div>
                    </div>
                </div>
                <div>
                    <img
                        src="/images/Coporate Training/doc2.jpeg"
                        className="size-full object-cover"
                        alt="Corporate Leadership Workshop"
                    />
                </div>
            </div>
        </section>
    );
}
